...
Architecture
------------
The public :func:`generate_extensions_config` function delegates to a
:class:`~pbxgen.pipeline.DialplanPipeline`.  The entire original generation
logic lives inside :class:`CoreDialplanPlugin` (priority 0), which seeds the
three core contexts (``internal``, ``from-trunk``, and ``ivr-<id>`` blocks).

External modules can inject additional behaviour by registering a plugin::
...
def _generate_ivr_context_body(menu: IVRMenu) -> str:
    """Generate IVR context body (without the ``[ivr-<id>]`` header line)."""
...
def _generate_ivr_context(menu: IVRMenu) -> str:
    """Generate complete IVR context including the ``[ivr-<id>]`` header."""
    return f"[ivr-{menu.id}]\n" + _generate_ivr_context_body(menu)


# ---------------------------------------------------------------------------
# Context body builders (extracted so CoreDialplanPlugin can reuse them
# without duplicating the string-building logic)
# ---------------------------------------------------------------------------

_PREAMBLE = """; Auto-generated dialplan configuration
...
def _build_internal_body(
    ...
    """Build the raw body of the ``[internal]`` context (without its header)."""
    body = "; Internal Extension Dialing (PJSIP)\n"
    ...
    if ring_groups:
        for g in ring_groups:
            if not g.enabled:
                continue
            body += f"exten => {g.extension},1,NoOp(Ring Group {g.name})\n"
            body += f" same => n,Set(CALLERID(name)=${{CALLERID(name)}})\n"
            body += _generate_ring_group_logic(g)
            body += "\n\n"
    ...
    if ivr_menus:
        for m in ivr_menus:
            if not m.enabled:
                continue
            body += f"exten => {m.extension},1,NoOp(IVR {m.name})\n"
            body += f" same => n,Goto(ivr-{m.id},s,1)\n\n"

    # Internal extension dialing pattern
    body += "exten => _1XXX,1,NoOp(Internal Call from ${CALLERID(all)} to ${EXTEN})\n"
    body += " same => n,Set(CALLERID(name)=${CALLERID(name)})\n"
        ...
        for p in peers:
            try:
                if p.enabled and getattr(p, "blf_enabled", True):
                    body += f"exten => {p.extension},hint,PJSIP/{p.extension}\n"
            except Exception:
                continue
    ...
    body += _generate_dial_logic("${EXTEN}", {}, 20)
    body += "\n\n"

    # Generate per-extension overrides
    for ext in sorted(override_extensions):
        ext_ring = ring_timeout_map.get(ext, 20)
        body += f"; Extension {ext} - custom rules\n"
        body += f"exten => {ext},1,NoOp(Call to {ext} with forwarding)\n"
        body += f" same => n,Set(CALLERID(name)=${{CALLERID(name)}})\n"
        body += _generate_dial_logic(ext, fwd_map, ext_ring)
        body += "\n\n"

    # === Outbound calling ===
    if outbound_map:
        body += "; === Outbound calling via assigned trunks ===\n"
        # Match external numbers: 0X. (national/international German dialing)
        body += "exten => _0X.,1,NoOp(Outbound call from ${CHANNEL(endpoint)} to ${EXTEN})\n"
        for ext in outbound_map:
            body += f' same => n,GotoIf($["${{CHANNEL(endpoint)}}x" = "{ext}x"]?out-{ext})\n'
        body += " same => n,NoOp(No outbound route for this extension)\n"
        body += " same => n,Playback(ss-noservice)\n"
        body += " same => n,Hangup()\n"
        for ext, info in outbound_map.items():
            ...
            if trunk and trunk.provider == "telekom_allip":
                ...
                body += f" same => n,Set(CALLERID(num)={allip_num})\n"
                body += f" same => n,Set(PJSIP_HEADER(add,P-Preferred-Identity)=<sip:{allip_num}@tel.t-online.de>)\n"
            else:
                body += f" same => n,Set(CALLERID(num)={route.did})\n"
                if pai:
                    pai_domain = trunk.sip_server if trunk else "localhost"
                    body += f" same => n,Set(PJSIP_HEADER(add,P-Asserted-Identity)=<sip:{pai}@{pai_domain}>)\n"
            body += f" same => n,Dial(PJSIP/${{EXTEN}}@trunk-ep-{tid},120,tT)\n"
            body += f" same => n,Hangup()\n"
        body += "\n"

        # Also match + prefixed numbers (international with +)
        body += "; International with + prefix\n"
        body += "exten => _+X.,1,NoOp(Outbound intl call from ${CHANNEL(endpoint)} to ${EXTEN})\n"
        for ext in outbound_map:
            body += f' same => n,GotoIf($["${{CHANNEL(endpoint)}}x" = "{ext}x"]?out-{ext})\n'
        body += " same => n,Playback(ss-noservice)\n"
        body += " same => n,Hangup()\n"
    ...
    body += """; Voicemail access - dial *98 to check voicemail
...
def _build_from_trunk_body(
    ...
    """Build the raw body of the ``[from-trunk]`` context (without its header)."""
    body = "; Inbound DID routing - auto-generated\n"
...
"""
    ...
    if routes:
        for route in routes:
            ...
            ext = route.destination_extension
            body += f"\n; {desc}\n"
            body += f"exten => {route.did},1,NoOp(Inbound call to DID {route.did})\n"
            body += f" same => n,Set(CALLERID(name)=${{CALLERID(name)}})\n"
            ...
            if ivr and ivr.enabled:
                body += " same => n,Answer()\n"
                body += " same => n,Wait(0.5)\n"
                body += f" same => n,Goto(ivr-{ivr.id},s,1)\n"
            elif rg and rg.enabled:
                body += " same => n,Answer()\n"
                body += " same => n,Wait(0.5)\n"
                body += _generate_ring_group_logic(rg)
            else:
                ext_ring = ring_timeout_map.get(ext, 20)
                body += _generate_dial_logic(ext, fwd_map, ext_ring, early_answer=True)
            body += "\n"
    else:
        body += """
...
# ---------------------------------------------------------------------------
# CoreDialplanPlugin – seeds the dialplan IR with all built-in contexts
# ---------------------------------------------------------------------------

class CoreDialplanPlugin:
    """Populates the dialplan IR with the core GonoPBX configuration.

    This plugin contains the baseline dialplan logic previously embedded
    inside :func:`generate_extensions_config`.  External modules **must**
    use a priority > 0 so they run after (and can reference or amend) the
    core contexts.

    Hook points available to external plugins after contribute()
    -----------------------------------------------------------
    * ``ops.add_include("internal", "my-context")``   – wire a new context
      into the main internal dialplan
    * ``ops.append_raw_lines("internal", "...")``      – append raw dialplan
      lines to the internal context
    * ``ops.append_raw_lines("from-trunk", "...")``    – append to inbound
      routing context
    * ``ops.append_step("my-context", "_X.", Step(...))`` – add structured
      steps to a module-owned context
    """

    priority: int = 0

    def contribute(self, ops, ctx) -> None:
        fwd_map = _build_forward_map(ctx.forwards)
        outbound_map = _build_outbound_map(ctx.routes, ctx.peers)
        trunk_map = {t.id: t for t in ctx.trunks}
        ring_timeout_map = _build_ring_timeout_map(ctx.mailboxes)
        ring_group_map = {g.extension: g for g in (ctx.ring_groups or [])}
        ivr_map = {m.extension: m for m in (ctx.ivr_menus or [])}

        ops.set_preamble(_PREAMBLE)

        internal_body = _build_internal_body(
            ctx.peers,
            ctx.ring_groups,
            ctx.ivr_menus,
            fwd_map,
        ...
        ops.set_raw_lines("internal", internal_body)

        from_trunk_body = _build_from_trunk_body(
            ctx.routes,
            fwd_map,
            ring_group_map,
            ivr_map,
        ...
        ops.set_raw_lines("from-trunk", from_trunk_body)

        for menu in (ctx.ivr_menus or []):
            if menu.enabled:
                ops.set_raw_lines(f"ivr-{menu.id}", _generate_ivr_context_body(menu))

    def patch(self, ops, ctx) -> None:
        pass
...
def generate_extensions_config(routes: List[InboundRoute], forwards: Optional[List[CallForward]] = None, mailboxes: Optional[List[VoicemailMailbox]] = None, peers: Optional[List[SIPPeer]] = None, trunks: Optional[List[SIPTrunk]] = None, ring_groups: Optional[List[RingGroup]] = None, ivr_menus: Optional[List[IVRMenu]] = None) -> str:
    ...
    Delegates to :class:`~pbxgen.pipeline.DialplanPipeline` with
    :class:`CoreDialplanPlugin` (priority 0) plus any globally registered
    plugins.  The returned string is functionally identical to the previous
    direct implementation; callers do not need to change.
    """
    from pbxgen.interfaces import GenerationContext
    from pbxgen.pipeline import DialplanPipeline, get_registered_plugins
    ...
    gen_ctx = GenerationContext(
        ...
        ivr_menus=ivr_menus or [],
    )

    pipeline = DialplanPipeline(
        plugins=[CoreDialplanPlugin()] + get_registered_plugins()
    )
    return pipeline.run(gen_ctx)
